...
export default function ProjectAdd({ devs, projects }: ProjectAdd) {
  ...
  const isDisabled = !project.project_name.trim() || tasks.every((t) => t.task_name.trim() === "") || !project.start_date || !project.end_date || project.members.length === 1;
          ...
          <div>
            ...
            {tasks.map((t, index) => (
              <div key={index} className="flex items-center justify-between my-2">
                ...
                <Input name={`task_name_${index}`} style={{ width: "70%", padding: "8px" }} value={t.task_name} onChange={(e) => handleTasksChange(index, e.target.value)} />
              </div>
            ))}
          </div>
          <div>
            <div className="flex items-center justify-between">
              ...
              {devs.length > 0 && (
                <Select className="w-[60%] p-1" isMulti name="select_dev" options={devs.map((devs) => ({ value: devs.id, label: devs.employee_name }))} onChange={handleSelectChange} />
              )}
            </div>
