...
export async function POST(req) {
    try {
        ...
        if (action === "updateTasks") {
            ...
            const [existedNotice] = await db.query(`SELECT * FROM notice_devlog WHERE leader_id = ? AND employee_id = ? AND project_id = ?`, [userId, accountId, projectId]);

            if (existedNotice && existedNotice.length > 0) {
                await db.query(`UPDATE notice_devlog SET notice_count = notice_count + 1, date = ? WHERE leader_id = ? AND employee_id = ? AND project_id = ?`, [date, userId, accountId, projectId])
            } else {
                await db.query(`INSERT INTO notice_devlog (leader_id, employee_id, project_id, date, notice_count) VALUES (?,?,?,?,?)`, [userId, accountId, projectId, date, 1]);
            }
